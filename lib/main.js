...
function onExamineResponse(event) {
  ...
  */
  ...
  console.log("dumping certificate chain...");
  for (let c = cert; c != null; c = c.issuer) {
    console.log("issuer: " + c.issuerName);
    console.log("serial number: " + c.serialNumber);
    console.log("SHA1 fingerprint: " + c.sha1Fingerprint);
  }
  console.log("end of certificate chain");

  /* TODO this is where we'd use the SHA1 fingerprint to find the
   * GCS of the parent of the cert (or of the root cert?) cert, then
   * hash the serial number we're looking for (example at
   * dom/activities/src/ActivitiesService.jsm:77), and look for the
   * hash in the GCS.
   */
}
